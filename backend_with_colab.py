...
COLAB_API_URL = os.getenv("COLAB_API_URL", "")  # 예: https://xxxx.ngrok.io
...
def call_colab_api(cameo_results: List[dict], timeout: int = 300) -> dict:  # 5분으로 증가
    ...
    try:
        print(f"[Colab] Calling Colab API at {COLAB_API_URL}")
        print(f"[Colab] Sending {len(cameo_results)} results")

        # Colab 헬스 체크
        print("[Colab] Checking Colab health...")
        ...
        if health_response.status_code != 200:
            print(f"[Colab] [FAIL] Health check failed: {health_response.status_code}")
            return {
                "success": False,
                "error": "Colab server not healthy"
            }

        print("[Colab] [OK] Health check passed")

        # AI 분석 요청
        print("[Colab] Sending analysis request...")
        ...
        if response.status_code == 200:
            ...
            print(f"[Colab] [ERROR] Error response: {error_detail}")
        ...
        print("[Colab] [TIMEOUT] Request timeout")
        return {
            "success": False,
        ...
        print(f"[Colab] [ERROR] Unexpected error: {e}")
        import traceback
        traceback.print_exc()
        return {
            "success": False,
...
async def health_check():
    ...
    if COLAB_API_URL:
        try:
            response = requests.get(f"{COLAB_API_URL}/health", timeout=3)
            if response.status_code == 200:
    ...
    return {
        ...
        "colab_url": COLAB_API_URL if COLAB_API_URL else "Not set"
    }
...
async def set_colab_url(url: str):
    ...
    return {
        "success": True,
        "message": f"Colab URL updated to: {COLAB_API_URL}"
    }
...
def call_colab_api_for_summary(analysis_result: dict, timeout: int = 120) -> dict:
    ...
    try:
        ...
        response = requests.post(
            f"{COLAB_API_URL}/summarize",  # 새로운 엔드포인트
...
if __name__ == "__main__":
    ...
    # .env 파일에서 COLAB_API_URL 로드 확인
    if COLAB_API_URL:
        print(f"[OK] Colab API URL configured: {COLAB_API_URL}")
    else:
        print("[WARNING] Colab API URL not set. AI analysis will be unavailable.")
        print("         Set COLAB_API_URL in .env or use POST /set-colab-url")
